...
- Create `artifactDir/` at the start of the run if it doesn't exist
...
5. **Validate anchors**: Every anchor must exist in `/tourkit/maps/tour.map.json`. If the map doesn't exist yet, warn but continue (the map is generated in Prompt 02C).
...
Checks (run all, report all issues, don't stop at first):
...
3. For each route:
   ...
   - Handle auth-gated routes: if a route redirects to login, note it and skip (don't fail)
